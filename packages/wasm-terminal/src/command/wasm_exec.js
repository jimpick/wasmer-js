...
;(() => {
  ...
  if (!global.fs) {
    ...
    global.fs = {
      ...
      writeSync (fd, buf) {
        ...
        if (nl != -1) {
          console.log('Jim writeSync', fd, outputBuf.substr(0, nl))
  ...
  global.Go = class {
    constructor (options) {
      ...
      if (options && options.wasiProxy) {
        this.wasiProxy = options.wasiProxy
      }
      if (options && options.wasiProxyImports) {
        this.wasiProxyImports = options.wasiProxyImports
      }
      ...
      const getiovs = (iovs, iovsLen) => {
        ...
        const buffers = Array.from({ length: iovsLen }, (_, i) => {
          const ptr = iovs + i * 8
          const buf = wasiProxy.view.getUint32(ptr, true)
          const bufLen = wasiProxy.view.getUint32(ptr + 4, true)
          return new Uint8Array(wasiProxy.memory.buffer, buf, bufLen)
        })
        return buffers
      }
      ...
      this.importObject = {
        go: {
          ...
          'runtime.wasmWrite': sp => {
            ...
            console.log('Jim runtime.wasmWrite', fd, p, n)
            fs.writeSync(
              fd,
