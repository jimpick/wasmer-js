...
export default class WASICommand extends Command {
  ...
  async run (wasmFs: WasmFs) {
    ...
    try {
      ...
      importsProxy = wasi.getImports(wasmModuleProxy)
    ...
    const importsWasi = imports.wasi_snapshot_preview1 || imports.wasi_unstable
    const importsProxyWasi =
      importsProxy.wasi_snapshot_preview1 || importsProxy.wasi_unstable
    const old_fd_write = importsWasi.fd_write
    importsWasi.fd_write = function (
      fd: number,
      iovs: number,
      iovsLen: number,
      nwritten: number
      ...
      importsProxyWasi.fd_write(fd, newIovs, iovsLen, newNwritten)
    }
